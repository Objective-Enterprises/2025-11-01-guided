...
function displayCharacters() {
    for (const character of characters) {
        ...
        const spells = character.spells.length === 0
            ? 'no spells'
            : `the spells ${character.spells.join(' and ')},`;
        const wand = character.wand[0] === 'None'
            ? 'no wand'
            : `whose wand has a core of ${character.wand[0]}, a length of ${character.wand[1]}, and a material of ${character.wand[2]}`;
        const message = `${character.name} is a ${character.age} year old ${wizard} from house ${character.house} with ${spells} and ${wand}.`;
        console.log(message);
    }
}
